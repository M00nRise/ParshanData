...
def get_query_url(query):
    return "https://www.google.co.il/search?q=%s&source=lnms&tbm=isch" % query
...
def refineHebrew(s):
    longList=str(s.encode('utf-8')).split(r'\x')[1:]
    longList[-1]=longList[-1][:-1]
    up=[x.upper() for x in longList]
    refined='%'.join(['']+up)
    print(refined)
    return refined
